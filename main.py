...
# Définition des crédits nécessaires pour utiliser les commandes
CREDITS_PER_LOOKUP = 1
BLACKLIST_FILE = "blacklist.json"


# Charger les données des crédits depuis un fichier JSON
def load_credits():
...
# Enregistrer les données des crédits dans un fichier JSON
def save_credits(credits):
    with open("credits.json", "w") as f:
        json.dump(credits, f, indent=4)


# Charger la liste de blacklist depuis un fichier JSON
def load_blacklist():
...
# Sauvegarder la liste de blacklist dans un fichier JSON
...
# Supprime la commande help existante
...
async def unbl(ctx, discord_id: int):
    ...
    allowed_role_id = 1250475866769592350  # ID du rôle autorisé
...
async def lookup(ctx, discord_id: str):
    # Vérifiez si l'ID est un ID Discord valide
    ...
    # Récupérez l'utilisateur et son avatar
    user = await fetch_user(discord_id)
    if user:
        avatar_url = user.avatar.url  # Utilisez avatar.url pour obtenir l'URL de l'avatar
    else:
        avatar_url = "https://cdn.discordapp.com/embed/avatars/0.png"  # URL d'avatar par défaut si l'utilisateur n'est pas trouvé
    ...
    async def search_file(file_path, discord_id):
        try:
            ...
            # Ignorer les fichiers non trouvés
            pass
    ...
    if results:
        ...
        def create_embed(page):
            ...
            embed.set_footer(text=f"Crédits restants : {remaining_credits} | Page: {page + 1}/{len(results)} | © Cfxdata ")
...
async def balance(ctx, member: discord.Member = None):
    ...
    embed = discord.Embed(title="Nombre de crédit au total :", color=0x808080)
...
async def add(ctx, member: discord.Member, amount: int):
    ...
    allowed_role_id = 1267507880337215551  # ID du rôle autorisé
    allowed_role = discord.utils.get(ctx.guild.roles, id=allowed_role_id)
...
# Définissez la commande help personnalisée
@bot.command()
async def help(ctx):
    embed = discord.Embed(title="Liste Des Commandes", description="**Voici les Commandes disponible de ce bot :**",
                          color=0x00000)
    embed.set_thumbnail(
        url="https://cdn.discordapp.com/attachments/1270177200855060542/1278501059710550096/cfxdata2.png?ex=66d10866&is=66cfb6e6&hm=49c78302e8733e782fd3f146c866fcb626bf681a646382bf4080956afdcbdb8d&")
    embed.set_footer(text="Veuillez utiliser | ! | pour chaque commandes ! | © Cfxdata")
...
# Fonction pour vérifier et déduire les crédits
...
@bot.event
async def Streaming():
    # Définir l'activité du bot sur Twitch
    activity = discord.Streaming(name=".gg/cfxdata ", url="https://www.twitch.tv/pornhub")
    await bot.change_presence(activity=activity)
    print(f'Logged in as {bot.user.name}')
    await update_status_message()


# Remplacer 'TOKEN' par le jeton de votre bot
bot.run('')
